...
function TablePagination({
  ...
  const GlobalFilter = ({ preGlobalFilteredRows, globalFilter, setGlobalFilter }) => {
    ...
    return (
      <div
        className={
          Actions !== undefined ? "flex flex-row justify-between" : "flex flex-col"
        }
      >
...
const Util = {
  ...
  funEach: function (obj, fun) {
    ...
    if (Util.isArray(obj)) {
      // Iterate over array
      for (var i = 0, l = obj.length; i < l; i++) fun.call(null, obj[i], i, obj);
    } else {
      // Iterate over object
      for (var key in obj)
...
exports.roleDatatable = async (page = 0, limit = 10, search = "", order = []) => {
  try {
...
module.exports = (type, message = "No desription", data = [], pagination = null) => {
  const ALLOWED_TYPES = [
    "VALID",
    "INVALID",
    "FOUND",
